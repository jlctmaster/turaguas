...
class Parroquia{
    ...
    public function cnombremunicipio(){
    	$Num_Parametro=func_num_args();
		if($Num_Parametro==0) return $this->cnombremunicipio;
     
		if($Num_Parametro>0){
	   		$this->cnombremunicipio=func_get_arg(0);
	 	}
    }
    ...
    public function Desactivar($user){
    	$sqlx="SELECT * FROM general.tlocalidad p WHERE nid_localidad = '$this->nid_localidad' AND 
		(EXISTS (SELECT 1 FROM general.tpersona per WHERE per.nid_localidad = p.nid_localidad) 
		OR EXISTS (SELECT 1 FROM general.tdireccion_despacho dp WHERE dp.nid_localidad = p.nid_localidad) 
		OR EXISTS (SELECT 1 FROM seguridad.tsistema s WHERE s.nid_localidad = p.nid_localidad) 
		)";
    ...
    public function Actualizar($user){
   	...
   	public function Consultar(){
	    $sql="SELECT p.nid_localidad,p.cdescripcion,p.nid_localidad_padre,p.dfecha_desactivacion,m.cdescripcion cnombremunicipio,
	    (CASE WHEN p.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END) AS estatus_parroquia 
	    FROM general.tlocalidad p 
	    INNER JOIN general.tlocalidad m ON p.nid_localidad_padre = m.nid_localidad 
		WHERE p.cdescripcion='$this->cdescripcion' AND p.ctabla='$this->ctabla'";
		$query=$this->pgsql->Ejecutar($sql);
	    if($this->pgsql->Total_Filas($query)!=0){
			$localidad=$this->pgsql->Respuesta($query);
			$this->nid_localidad($localidad['nid_localidad']);
			$this->cnombremunicipio($localidad['cnombremunicipio']);
			$this->cdescripcion($localidad['cdescripcion']);
			$this->nid_localidad_padre($localidad['nid_localidad_padre']);
		   	$this->estatus_parroquia($localidad['estatus_parroquia']);
