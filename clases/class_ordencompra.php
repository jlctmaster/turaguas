...
class Documento{
	...
	public function GenerarNroSolicitud(){
		...
		if($row=$this->pgsql->Respuesta($query)){
			...
			return $this->nnro_solicitud(($numero));
		}else{
			return 0;
		}
	}
   	...
   	public function Registrar($user){
	    $sql="INSERT INTO facturacion.tdocumento (nnro_orden,ctipo_transaccion,nid_tipodocumento,dfecha_documento,crif_persona,nid_condicionpago,nid_almacen
	    ,cestado_documento,caccion_documento,nmonto_total,dcreado_desde,ccreado_por,dmodificado_desde,cmodificado_por) VALUES 
	    ('$this->nnro_orden','C','$this->nid_tipodocumento','$this->dfecha_documento','$this->crif_persona','$this->nid_condicionpago','$this->nid_almacen'
    ...
    public function Actualizar($user){
   	...
   	public function Consultar(){
		...
		WHERE nnro_orden='$this->nnro_orden' AND nnro_ent_recib IS NULL AND d.ctipo_transaccion = 'C' 
   	...
   	public function Comprobar(){
	    ...
	    AND nnro_ent_recib IS NULL AND ctipo_transaccion = 'C'";
		$query=$this->pgsql->Ejecutar($sql);
	    if($this->pgsql->Total_Filas($query)!=0){
			$documento=$this->pgsql->Respuesta($query);
			$this->nid_documento($documento['nid_documento']);
