...
class Documento{
	...
	public function dfecha_desactivacion(){
   	...
   	public function Registrar($user){
	    $sql="INSERT INTO facturacion.tdocumento (nnro_ent_recib,dfecha_ent_recib,nnro_orden,ctipo_transaccion,nid_tipodocumento,dfecha_documento,crif_persona,nid_direcciondespacho
	    ...
	    ('$this->nnro_ent_recib','$this->dfecha_ent_recib','$this->nnro_orden','V','$this->nid_tipodocumento','$this->dfecha_documento','$this->crif_persona',$this->cdirecciondespacho
	    ...
	    if($this->pgsql->Ejecutar($sql)!=null){
			...
			WHERE d.nnro_orden = '$this->nnro_orden' AND d.ctipo_transaccion = 'V'";
	    	...
	    	while ($obj=$this->pgsql->Respuesta($query)){
				...
				((SELECT nid_documento FROM facturacion.tdocumento WHERE nnro_ent_recib = '$this->nnro_ent_recib' AND ctipo_transaccion = 'V'),'".$obj['nlinea']."'
    ...
    public function Actualizar($user){
	    ...
	    ,crif_persona='$this->crif_persona',nid_direcciondespacho=$this->cdirecciondespacho,nid_condicionpago='$this->nid_condicionpago',nid_almacen='$this->nid_almacen',nmonto_total='$this->nmonto_total'
   	...
   	public function Consultar(){
		...
		WHERE d.nnro_ent_recib='$this->nnro_ent_recib' AND d.ctipo_transaccion = 'V' 
   	...
   	public function Comprobar(){
	    $sql="SELECT * FROM facturacion.tdocumento WHERE nnro_ent_recib='$this->nnro_ent_recib' AND ctipo_transaccion = 'V'";
		$query=$this->pgsql->Ejecutar($sql);
	    if($this->pgsql->Total_Filas($query)!=0){
			$documento=$this->pgsql->Respuesta($query);
			$this->nid_documento($documento['nid_documento']);
	...
	public function GenerarMovimientoInventario($documento,$fecha,$tipo,$user){
		...
		if($this->pgsql->Ejecutar($sql)!=null){
			...
			WHERE ctabla = 'tinventario' AND (LOWER(cdescripcion) LIKE '%terminado%' OR LOWER(cdescripcion) LIKE '%productos terminados%'))";
			$query = $this->pgsql->Ejecutar($sqlx);
   	...
   	public function BuscarDatosNroOrden($nnro_orden){
   		$sql="SELECT TO_CHAR(d.dfecha_documento,'DD/MM/YYYY') dfecha_documento,p.crif_persona,p.cnombre,p.cdireccion,d.nid_direcciondespacho,d.nid_almacen,
        ...
        LEFT JOIN facturacion.vw_orden_entrega_recibo de ON de.nnro_orden = d.nnro_orden and d.cid_articulo = de.cid_articulo
   		WHERE d.nnro_orden = '$nnro_orden' AND d.ctipo_transaccion = 'V' 
   		GROUP BY d.dfecha_documento,p.crif_persona,p.cnombre,p.cdireccion,d.nid_direcciondespacho,d.nid_almacen,p.nid_condicionpago";
		$query = $this->pgsql->Ejecutar($sql);
        while($Obj=$this->pgsql->Respuesta_assoc($query)){
                $rows[]=array_map("html_entity_decode",$Obj);
            }
	   		...
	   		if($this->pgsql->Total_Filas($query)!=0){
		        ...
		        if($this->pgsql->Ejecutar($sql)!=null){
		        	if($estatus=="CO"){
			        	...
			        	$this->GenerarMovimientoInventario($documento,$fecha,'S',$user);
		        	}
		            $rows[] = array("msj" => $msj);
		            $json = json_encode($rows);
		        }
	   		...
	   		if($this->pgsql->Total_Filas($query)==0){
		   		...
		   		if($this->pgsql->Ejecutar($sql)!=null){
		        	...
		        	while($lineas=$this->pgsql->Respuesta($query1)) {
		        		...
		        		if($listo==true){
		        			$this->GenerarMovimientoInventario($documento,'E',$user);
		        		}
		        	}
		            $rows[] = array("msj" => $msj);
		            $json = json_encode($rows);
	   			...
	   			$rows[] = array("msj" => "Error el documento esta asociado a una orden de despacho por lo tanto no se puede anular, primero anule la orden de despacho.");
		        $json = json_encode($rows);
	   		}
   		}
        return $json;
