...
function init(){
	...
	function accionDocumento(estatus){
		...
		if(estatus=="VO"){
			alertDGC(document.getElementById('Anular'),'./menu_principal.php?ordencompra');
			...
			$('#BtnAnular').click(function(){
				...
				$.ajax({
			        url: '../controladores/control_ordencompra.php',
			        type: 'POST',
			        async: true,
			        data: Datos2,
			        dataType: "json",
	...
	$('#tab-lineaordencompra').click(function(){
	...
	// Para las gestión de compra el número del documento no es generado automaticamente 
	// ya que es el proveedor quien nos lo facilita.
	//Buscar nro de orden.
    ...
    function BuscarPrecioLista(value){
        $.ajax({
        url: '../controladores/control_lineaordencompra.php',
        type: 'POST',
        async: true,
        data: value,
        dataType: "json",
...
function validar_formulario(param,urltab){
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "ordencompra") || (devuelve_boton(param)=="Modificar" && urltab == "ordencompra")){
		if(valor1.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese el nro de la órden de compra')
			permitido=false;
			...
			alert('Seleccione la fecha de la órden de compra')
			permitido=false;
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "lineaordencompra") || (devuelve_boton(param)=="Modificar" && urltab == "lineaordencompra")){
		if(valor8.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese la línea de la orden de compra')
			permitido=false;
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "ordencompra"){
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "lineaordencompra"){
	...
	if(permitido==true && urltab =="ordencompra"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
		document.getElementById("form1").submit();
	}
	else if(permitido==true && urltab =="lineaordencompra"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
