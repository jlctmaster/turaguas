...
function init(){
    ...
    function BuscarDatosNroDevolucion(value){
        $.ajax({
        url: '../controladores/control_reembolsoproveedor.php',
        type: 'POST',
        async: true,
        data: value,
        dataType: "json",
...
function validar_formulario(param,urltab){
	...
	if(devuelve_boton(param)=="Registrar" && urltab == "reembolsoproveedor"){
	...
	//Validar campos antes de insertar o modificar por pestañas.
	if(devuelve_boton(param)=="Modificar" && urltab == "reembolsoproveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "reembolsoproveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	...
	if(permitido==true && urltab =="reembolsoproveedor"){
