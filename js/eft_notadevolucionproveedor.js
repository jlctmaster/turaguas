function validar_formulario(param,urltab){
	...
	valor0=document.getElementById('nnro_recepcion').value;
	...
	if(devuelve_boton(param)=="Registrar" && urltab == "notadevolucionproveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length>0){ //para no permitir que se quede en blanco
			...
			alert('Seleccione un Nro. de Recepción')
			permitido=false;
		}
			...
			for(j=0;j<valor2.length;j++){
				...
				if(parseInt(CN)>parseInt(CV)){
					...
					alert("Seleccione un motido de devolución en la linea "+Ln);
					permitido=false;
				}
			}
	...
	if(devuelve_boton(param)=="Modificar" && urltab == "notadevolucionproveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "notadevolucionproveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	...
	if(permitido==true && urltab =="notadevolucionproveedor"){
