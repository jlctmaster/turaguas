...
function init(){
	$('#tab-contacto').click(function(){
		if($(location).attr('search')!="?proveedor" && $(location).attr('hash')!="#proveedor"){
			return true;
		}
		else{
			if($('#crif_personaproveedor').val()==""){
...
function validar_formulario(param,urltab){
	...
	permitido=true;
	//campos pestaña 1
	valor=document.getElementById('crif_personaproveedor').value;
	valor2=document.getElementById('cnombreproveedor').value;
	valor3=document.getElementById('ctelefhabproveedor').value;
	valor1=document.getElementById('cemailproveedor').value;
	valor4=document.getElementById('nid_condicionpago').value;
	valor5=document.getElementById('nid_localidadproveedor').value;
	valor6=document.getElementById('cdireccionproveedor').value;
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "proveedor") || (devuelve_boton(param)=="Modificar" && urltab == "proveedor")){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese el RIF del proveedor')
			permitido=false;
		}
		else if(valor1.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese la dirección de correo del proveedor')
			permitido=false;
		}
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "proveedor"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	...
	if(permitido==true && urltab =="proveedor"){
