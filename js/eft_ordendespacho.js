...
function init(){
	...
	function accionDocumento(estatus){
		...
		if(estatus=="VO"){
			alertDGC(document.getElementById('Anular'),'./menu_principal.php?ordendespacho');
			...
			$('#BtnAnular').click(function(){
				...
				$.ajax({
			        url: '../controladores/control_ordendespacho.php',
			        type: 'POST',
			        async: true,
			        data: Datos2,
			        dataType: "json",
	...
	$('#tab-lineaordendespacho').click(function(){
    ...
    function BuscarDatosNroOrden(value){
        ...
        success: function(resp){
        	...
        	comboDependiente(Datos,'../controladores/control_ordendespacho.php',$('#cdirecciondespacho'));
        },
        error: function(resp){
        	alert('Error al procesar la petición')
        }
    ...
    function BuscarPrecioLista(value){
        $.ajax({
        url: '../controladores/control_lineaordendespacho.php',
        type: 'POST',
        async: true,
        data: value,
        dataType: "json",
...
function validar_formulario(param,urltab){
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "ordendespacho") || (devuelve_boton(param)=="Modificar" && urltab == "ordendespacho")){
		if(valor1.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese el nro de la órden de despacho')
			permitido=false;
			...
			alert('Seleccione la fecha de la órden de despacho')
			permitido=false;
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "lineaordendespacho") || (devuelve_boton(param)=="Modificar" && urltab == "lineaordendespacho")){
		if(valor8.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese la línea de la orden de despacho')
			permitido=false;
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "ordendespacho"){
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "lineaordendespacho"){
	...
	if(permitido==true && urltab =="ordendespacho"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
		document.getElementById("form1").submit();
	}
	else if(permitido==true && urltab =="lineaordendespacho"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
