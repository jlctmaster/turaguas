...
valor=document.getElementById('cdescripcion').value;
	valor2=document.getElementById('nid_condicionpago').value;
if((devuelve_boton(param)=="Registrar" && urltab == "condicionpago") || (devuelve_boton(param)=="Modificar" && urltab == "condicionpago")){
	if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se queda en blanco
	alert('Ingrese la condición de pago')
	permitido=false;
	}
	else if(valor.replace(/^\s+|\s+$/gi,"").length<=3){ 
	alert('La condición de pago debe ser mayor a 3 caracteres.')
	permitido=false;
	}
}
...
if(devuelve_boton(param)=="Consultar" && urltab == "tipodocumento"){
		if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se queda en blanco
			alert('Debe ingresar la condición de pago')
			permitido=false;
		}
}
	
if(devuelve_boton(param)=="Desactivar" && urltab == "condicionpago"){
...
if(permitido==true && urltab =="condicionpago"){
