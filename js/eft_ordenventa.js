...
function init(){
	...
	function accionDocumento(estatus){
		...
		if(estatus=="VO"){
			alertDGC(document.getElementById('Anular'),'./menu_principal.php?ordenventa');
			...
			$('#BtnAnular').click(function(){
				...
				$.ajax({
			        url: '../controladores/control_ordenventa.php',
			        type: 'POST',
			        async: true,
			        data: Datos2,
			        dataType: "json",
	...
	$('#tab-lineaordenventa').click(function(){
	...
	$('#nid_tipodocumento').change(function(){
		var nro_orden = $('#nnro_orden').val();
		if(nro_orden.length==0 && $('#nid_tipodocumento').val()!=0){
    ...
    function BuscarPrecioLista(value){
        $.ajax({
        url: '../controladores/control_lineaordenventa.php',
        type: 'POST',
        async: true,
        data: value,
        dataType: "json",
    ...
    function BuscarDatosCliente(value){
        ...
        success: function(resp){
        	...
        	$('#cdireccioncliente').val(resp[0].cdireccion);
        	$('#nid_condicionpago').val(resp[0].nid_condicionpago);
        	var Datos = {"operacion":"BuscarDirecciones","filtro":resp[0].crif_persona};
        	comboDependiente(Datos,'../controladores/control_ordenventa.php',$('#cdirecciondespacho'));
        },
        error: function(resp){
        	alert('Error al procesar la petición')
        }
    ...
    function BuscarDireccionCliente(){
    	var Datos = {"operacion":"BuscarDirecciones","filtro":$('#crif_persona').val()};
    	comboDependiente(Datos,'../controladores/control_ordenventa.php',$('#cdirecciondespacho'));
    }

	setTimeout(function(){BuscarDireccionCliente()},500);
}
...
function validar_formulario(param,urltab){
	...
	valor1=document.getElementById('nnro_orden').value;
	valor2=document.getElementById('nid_tipodocumento').value;
	valor3=document.getElementById('dfecha_documento').value;
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "ordenventa") || (devuelve_boton(param)=="Modificar" && urltab == "ordenventa")){
		if(valor1.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese el nro de la órden de venta')
			permitido=false;
			...
			alert('Seleccione la fecha de la órden de venta')
			permitido=false;
	...
	if((devuelve_boton(param)=="Registrar" && urltab == "lineaordenventa") || (devuelve_boton(param)=="Modificar" && urltab == "lineaordenventa")){
		if(valor8.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			alert('Ingrese la línea de la orden de venta')
			permitido=false;
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "ordenventa"){
	...
	if(devuelve_boton(param)=="Desactivar" && urltab == "lineaordenventa"){
	...
	if(permitido==true && urltab =="ordenventa"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
		document.getElementById("form1").submit();
	}
	else if(permitido==true && urltab =="lineaordenventa"){
		document.getElementById("operacion_"+urltab).value=devuelve_boton(param);
