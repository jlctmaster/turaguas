...
**	Adaptación de Mysql a PostgreSQL
**	Versión 1.7.0 		(31/03/2014):	-Versión inicial en PostgreSQL.
*/

/*	Agregar Clase de Conexión y definir objeto para ejecutar los SQL.*/
include('../../clases/class_bd.php');
$conexion = new Conexion();

/*
**	Paginator 
**	Paginación de resultados de consultas a PGSql con PHP
**
**	Versión 1.6.0
**
**	Nombre de archivo	:
...
**									-Se optimizó el conteo del total de registros utilizando el COUNT(*) de PGSql.
...
**									-Se agregó la opción de elegir si se quiere mostrar los mensajes de error de PGSql.
...
**									 reportar el bug.
**	Versión 1.5.0 	(03/11/2004): 	-Se agregó la opción de elegir si se hace el conteo desde PGSql (COUNT(*)) o desde PHP (PGSql_num_rows()).
 ...
 * $_pagi_mostrar_errores		OPCIONAL		Booleano. Define si se muestran o no los errores de PGSql que se puedan producir.
 ...
 * $_pagi_conteo_alternativo	OPCIONAL		Booleano. Define si se utiliza PGSql_num_rows() (true) o COUNT(*) (false).
 ...
 */
 if(empty($_pagi_sql)){
	...
	// Este error se muestra sí o sí (ya que no es un error de PGSql)
 ...
 if(!isset($_pagi_conteo_alternativo)){
	...
	// Se realiza el conteo dese PGSql con COUNT(*)
...
**	Editado por Jorge Colmenarez
**	Metodos de la clase 
**	Ejecutar:	Ejecuta la consulta en la conexión de base de datos especificada / Executes the query on the specified database connection
**	Obtener_Resultado:	Devuelve los valores de un recurso de resultado / Returns values from a result resource
**	Total_Filas:	Devolverá el número de filas de un recurso de resultado PostgreSQL. / will return the number of rows in a PostgreSQL result resource.
*/
 ...
 */
 ...
 if($_pagi_conteo_alternativo == false){
 	...
 	$_pagi_result2 = $conexion->Ejecutar($_pagi_sqlConta);
	// Si ocurrió error y mostrar errores está activado
 	if($_pagi_result2 == false && $_pagi_mostrar_errores == true){
		die (" Error en la consulta de conteo de registros: $_pagi_sqlConta. PGSql dijo: <b>".pg_result_error($_pagi_result2)."</b>");
 	}
 	$_pagi_totalReg = $conexion->Obtener_Resultado($_pagi_result2,0,0);//total de registros
 }else{
	$_pagi_result3 = $conexion->Ejecutar($_pagi_sql);
	// Si ocurrió error y mostrar errores está activado
 	if($_pagi_result3 == false && $_pagi_mostrar_errores == true){
		die (" Error en la consulta de conteo alternativo de registros: $_pagi_sql. PGSql dijo: <b>".pg_result_error($_pagi_result3)."</b>");
 	}
	$_pagi_totalReg = $conexion->Total_Filas($_pagi_result3);
 }
...
//MIGUEL esto lo hago para saber si estoy en una página con el htaccess. 
...
if (!$_pagi_htaccess){
	...
	//MIGUEL voy a propagar las variables de paginación de una manera distinta
	// La variable $_pagi_navegacion contendrá los enlaces a las páginas.
	//echo "<p>_pagi_enlace = $_pagi_enlace</p>";
	 ...
	 //voy a ver si tenemos un pg_x porque esto no me interesa que esté en la variable $_pagi_enlace
 ...
 */
 ...
 $_pagi_result = $conexion->Ejecutar($_pagi_sqlLim);
 // Si ocurrió error y mostrar errores está activado
 if($_pagi_result == false && $_pagi_mostrar_errores == true){
 	die ("Error en la consulta limitada: $_pagi_sqlLim. PGSql dijo: <b>".pg_result_error($_pagi_result)."</b>");
 }
 ...
 */
 ...
 $_pagi_info = " <span class=fuente8><b>Se muestran desde el $_pagi_desde hasta el $_pagi_hasta de un total de $_pagi_totalReg páginas</b></span>";
 ...
 $_pagi_info_articulos = " <span class=fuente8><b>Encontrados $_pagi_totalReg artículos. Se muestran desde el $_pagi_desde hasta el $_pagi_hasta.</b></span>";
//------------------------------------------------------------------------
 ...
 * $_pagi_result		Identificador del resultado de la consulta a la BD para los registros de la página actual. 
 				Listo para ser "pasado" por una función como PGSql_fetch_row(), PGSql_fetch_array(), 
				PGSql_fetch_assoc(), etc.
