<?php
    ...
    public function Header(){
    	...
    	$pgsql=new Conexion();
    	$sql="SELECT DISTINCT TO_CHAR(r.dfecha_entrega,'DD/MM/YYYY') fecha,r.nnro_entrega 
		FROM facturacion.vw_entrega r 
		WHERE r.nnro_entrega = '".$_GET['nnro_entrega']."'";
		$data=$pgsql->Ejecutar($sql);
		$fila=array();
		while ($row=$pgsql->Respuesta($data)){
			$fila['fecha'][]=$row['fecha'];
			$fila['nnro_entrega'][]=$row['nnro_entrega'];
		}
		$r1  = $this->w - 81;
		$r2  = $r1 + 60;
		$y1  = 19;
		...
		$this->Cell(0,6,'NOTA DE ENTREGA',0,1,"C");
		$this->Ln(2);
		...
		$this->SetFont('Arial','',10);
		$this->SetTextColor(0,0,0);
		$this->Cell($anchura*2,$altura,$fila['nnro_entrega'][0],0,1,'L',$color_fondo);
	}
	...
	require_once("../clases/class_bd.php");
	$pgsql=new Conexion();
	$sql="SELECT DISTINCT e.nnro_cotizacion,e.nnro_factura,
	e.nlinea,e.cid_articulo,a.cdescripcion articulo,e.ncantidad_articulo,um.csimbolo,
	substr(e.crif_persona,1,1)||'-'||substr(e.crif_persona,2,8)||'-'||substr(e.crif_persona,9,1) crif_persona,
	...
	WHERE e.nnro_entrega = '".$_GET['nnro_entrega']."' 
	ORDER BY nlinea ASC";
	$i=-1;
	$data=$pgsql->Ejecutar($sql);
	if($pgsql->Total_Filas($data)!=0){
		...
		$lobjPdf->Cell($anchura*2,$altura,$filas['nnro_cotizacion'][0],0,0,'L',$color_fondo);
