<?php
    ...
    public function Header(){
    	...
    	$pgsql=new Conexion();
    	$sql="SELECT DISTINCT TO_CHAR(r.dfecha_documento,'DD/MM/YYYY') fecha,r.nnro_reembolso 
		FROM facturacion.vw_reembolso_cliente r 
		WHERE r.nnro_reembolso = '".$_GET['nnro_reembolso']."'";
		$data=$pgsql->Ejecutar($sql);
		$fila=array();
		while ($row=$pgsql->Respuesta($data)){
			$fila['fecha'][]=$row['fecha'];
			$fila['nnro_reembolso'][]=$row['nnro_reembolso'];
		}
		$r1  = $this->w - 81;
		$r2  = $r1 + 60;
		$y1  = 19;
		...
		$this->Cell(0,6,'REEMBOLSO A CLIENTE',0,1,"C");
		$this->Ln(2);
		...
		$this->SetFont('Arial','',10);
		$this->SetTextColor(0,0,0);
		$this->Cell($anchura*2,$altura,$fila['nnro_reembolso'][0],0,1,'L',$color_fondo);
	}
	...
	require_once("../clases/class_bd.php");
	$pgsql=new Conexion();
	$sql="SELECT DISTINCT TRIM(dp.nnro_devolucion) nnro_devolucion,TRIM(e.nnro_factura) nnro_factura,
	rp.nlinea,rp.cid_articulo,a.cdescripcion articulo,rp.ncantidad_articulo,
	CASE WHEN rp.ncantidad_articulo > 1 THEN pr.cdescripcion||'S' ELSE pr.cdescripcion END presentacion,
	substr(rp.crif_persona,1,1)||'-'||substr(rp.crif_persona,2,8)||'-'||substr(rp.crif_persona,9,1) crif_persona,
	...
	INNER JOIN facturacion.vw_entrega e ON dp.nid_documentoventa = e.nid_documentoventa 
	INNER JOIN inventario.tarticulo a ON rp.cid_articulo = a.cid_articulo 
	...
	WHERE rp.nnro_reembolso = '".$_GET['nnro_reembolso']."' 
	...
	if($pgsql->Total_Filas($data)!=0){
		...
		while($rows=$pgsql->Respuesta($data)){
			$filas['nnro_devolucion'][]=$rows['nnro_devolucion'];
		...
		$lobjPdf->SetFont('Arial','B',10);
		$lobjPdf->SetTextColor(0,0,0);
		$lobjPdf->Cell($anchura*2,$altura,'N° Devolución Asociada: ',0,0,'L',$color_fondo);
		$lobjPdf->SetFont('Arial','',9);
		$lobjPdf->SetTextColor(0,0,0); 
		$lobjPdf->Cell($avnzar+6);
		$lobjPdf->Cell($anchura*2,$altura,$filas['nnro_devolucion'][0],0,0,'L',$color_fondo);
		...
		$lobjPdf->Cell($avnzar);
		$lobjPdf->SetFont("arial","B",10);
		$lobjPdf->Row(array('N°','Cod. Artículo','Descripción','Cantidad','Presentación'),false);
		$lobjPdf->SetFont("arial","",10);
		$lobjPdf->Cell($avnzar);
		for($i=0;$i<count($filas['nnro_devolucion']);$i++){
			$lobjPdf->Row(array(
			$filas['nlinea'][$i],
