<?php
    ...
    public function Header(){
    	...
    	$pgsql=new Conexion();
    	$sql="SELECT DISTINCT TO_CHAR(dp.dfecha_devolucion,'DD/MM/YYYY') fecha,dp.nnro_devolucion 
		FROM facturacion.vw_devolucion_proveedor dp 
		WHERE dp.nnro_devolucion = '".$_GET['nnro_devolucion']."'";
		...
		$this->Cell(0,6,'NOTA DE DEVOLUCIÓN A PROVEEDOR',0,1,"C");
		$this->Ln(2);
	...
	require_once("../clases/class_bd.php");
	$pgsql=new Conexion();
	$sql="SELECT DISTINCT TRIM(r.nnro_solicitud) nnro_solicitud,TRIM(r.nnro_factura) nnro_factura, TRIM(r.nnro_recepcion) nnro_recepcion,
	substr(dp.crif_persona,1,1)||'-'||substr(dp.crif_persona,2,8)||'-'||substr(dp.crif_persona,9,1) crif_persona,
	p.cnombre,p.cdireccion,substr(p.ctelefhab,1,4)||'-'||substr(p.ctelefhab,5,7) ctelefhab,
	dp.nlinea,dp.cid_articulo,a.cdescripcion articulo,dp.ncantidad_articulo,pr.cdescripcion presentacion, 
	...
	if($pgsql->Total_Filas($data)!=0){
		...
		while($rows=$pgsql->Respuesta($data)){
			...
			$filas['nnro_recepcion'][]=$rows['nnro_recepcion'];
		...
		$lobjPdf->Cell($avnzar*1.61);
		$lobjPdf->Cell($anchura*2,$altura,'N° Nota de Recepción Asociada: ',0,1,'L',$color_fondo);
		$lobjPdf->SetFont('Arial','',9);
		$lobjPdf->SetTextColor(0,0,0); 
		$lobjPdf->Cell($avnzar*2);
		$lobjPdf->Cell($anchura*2,$altura,$filas['nnro_solicitud'][0],0,0,'C',$color_fondo);
		$lobjPdf->Cell($avnzar*1.5);
		$lobjPdf->Cell($anchura*2,$altura,$filas['nnro_factura'][0],0,0,'C',$color_fondo);
		$lobjPdf->Cell($avnzar*2);
		$lobjPdf->Cell($anchura*2,$altura,$filas['nnro_recepcion'][0],0,0,'C',$color_fondo);
		$lobjPdf->Ln(10);
		$lobjPdf->Cell($avnzar*1.61);
		$lobjPdf->SetFont('Arial','B',10);
		$lobjPdf->SetTextColor(0,0,0);
