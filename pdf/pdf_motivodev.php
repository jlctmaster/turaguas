<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS MOTIVOS DE DEVOLUCIÓN',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*4,$altura,'CÓDIGO MOTIVO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'MOTIVO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'CÓDIGO GRUPO',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'GRUPO DE MOTIVOS',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,40,30,40,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT e.nid_motivodevolucion,LOWER(e.cdescripcion) motivodev,
    e.nid_motivodevolucion_padre,LOWER(p.cdescripcion) grupo, 
    CASE WHEN e.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus
FROM facturacion.tmotivo_devolucion e INNER JOIN facturacion.tmotivo_devolucion p ON e.nid_motivodevolucion_padre = p.nid_motivodevolucion 
ORDER BY e.nid_motivodevolucion DESC";
   $i=-1;
  $data=$pgsql->Ejecutar($sql);
	...
	$prueba[] = $tperfil['estado'];
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_motivodevolucion']),
         ucwords($tperfil['motivodev']),
         ucwords($tperfil['nid_motivodevolucion_padre']),
         ucwords($tperfil['grupo']),
