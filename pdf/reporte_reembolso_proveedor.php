   ...
   //Cabecera de página
    ...
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'CANTIDAD DE REEMBOLSO POR PROVEEDOR',0,1,"C");
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*3.5,$altura,'NRO. REEMBOLSO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'FECHA REEMBOLSO.',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'NRO. DEVOLUCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'FECHA DEVOLUCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'PROVEEDOR',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CANT. ARTÍCULOS',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4.5,$altura,'MOTIVO REEMBOLSO',1,1,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(35,40,35,40,40,35,45));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT TRIM(dp.nnro_devolucion) nnro_devolucion, TO_CHAR(dp.dfecha_devolucion,'DD/MM/YYYY') dfecha_devolucion, 
          TRIM(rp.nnro_reembolso) nnro_reembolso, TO_CHAR(rp.dfecha_documento,'DD/MM/YYYY') dfecha_reembolso,
          COUNT(rp.cid_articulo) cantidad,p.crif_persona||' '||p.cnombre proveedor,md.cdescripcion motivoreembolso
          ...
          WHERE rp.dfecha_documento between '$dfecha_desde' and '$dfecha_hasta'
          GROUP BY rp.nnro_reembolso,rp.dfecha_documento,dp.nnro_devolucion,dp.dfecha_devolucion,
          p.crif_persona,p.cnombre,md.cdescripcion";   
$i=-1;
