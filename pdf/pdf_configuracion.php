<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LAS CONFIGURACIONES',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar); 
      $this->Cell($anchura*1,$altura,'CÓD',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'NOMBRE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2,$altura,'MÍN. CLAVE',1,0,'L',$color_fondo);
      $this->Cell($anchura*2.3,$altura,'MÁX. CLAVE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'LETRAS MAYÚS.',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'LETRAS MINÍS.',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CARACTERES NÚM',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CARACTERES ESP',1,0,'L',$color_fondo);
      $this->Cell($anchura*2,$altura,'VIG. CLAVE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2,$altura,'DÍAS AVISO',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(10,30,20,23,30,30,35,35,20,20,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT *, 
      CASE WHEN dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus  
      FROM seguridad.tconfiguracion 
      ORDER BY nid_configuracion DESC";
   $i=-1;
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         ucwords($tperfil['cdescripcion']),
         ucwords($tperfil['nlongitud_minclave']),
         ucwords($tperfil['nlongitud_maxclave']),
         ucwords($tperfil['ncantidad_letrasmayusculas']),
         ucwords($tperfil['ncantidad_letrasminusculas']),
         ucwords($tperfil['ncantidad_numeros']),
         ucwords($tperfil['ncantidad_caracteresespeciales']),
         ucwords($tperfil['ndias_vigenciaclave']),
         ucwords($tperfil['ndias_aviso']),
