<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LA BOTONERA',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'BOTÓN',1,0,'L',$color_fondo);
      $this->Cell($anchura*2,$altura,'ÓRDEN',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,30,20,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT *,CASE WHEN dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus 
    FROM seguridad.topcion
    ORDER BY nid_opcion DESC";
   $i=-1;
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_opcion']),
         ucwords($tperfil['cnombreopcion']),
         ucwords($tperfil['norden']),
