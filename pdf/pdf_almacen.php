<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS ALMACENES',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*4,$altura,'CÓDIGO ALMACEN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*5,$altura,'NOMBRE ALMACEN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'CÓDIGO UBICACIÓN',1,0,'L',$color_fondo);
      $this->Cell($anchura*5,$altura,'NOMBRE UBICACIÓN',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,50,40,50,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT u.nid_ubicacion,LOWER(u.cdescripcion) ubicacion,
    a.nid_almacen,LOWER(a.cdescripcion) almacen, 
    CASE WHEN a.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus
FROM inventario.tubicacion u 
INNER JOIN inventario.talmacen a ON u.nid_ubicacion = a.nid_almacen 
ORDER BY u.nid_ubicacion DESC";
   $i=-1;
  $data=$pgsql->Ejecutar($sql);
	...
	$prueba[] = $tperfil['estado'];
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_almacen']),
         ucwords($tperfil['almacen']),
         ucwords($tperfil['nid_ubicacion']),
