<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS FORMULARIOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar); 
      $this->Cell($anchura*4,$altura,'CÓDIGO FORMULARIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*6,$altura,'NOMBRE FORMULARIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'URL FORMULARIO',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'CÓDIGO MÓDULO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*6,$altura,'NOMBRE MÓDULO',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,60,40,40,60,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT f.nid_formulario,LOWER(f.cnombreformulario) as formulario, (f.curl) as url, m.nid_modulo,LOWER(m.cnombremodulo) as modulo, 
      CASE WHEN f.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus  
      FROM seguridad.tformulario f 
      INNER JOIN seguridad.tmodulo m ON m.nid_modulo = f.nid_modulo 
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_formulario']),
         ucwords($tperfil['formulario']),
         ucwords($tperfil['url']),
         ucwords($tperfil['nid_modulo']),
         ucwords($tperfil['modulo']),
