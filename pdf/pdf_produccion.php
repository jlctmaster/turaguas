<?php
  ...
  $this->Image("../images/logo2.jpg" ,125,75,45,90, "JPG" ,$_SERVER['HTTP_HOST']."/turaguas/vistas/menu_principal");
   ...
   $this->Cell(0,6,'LISTADO DE LAS PRODUCCIONES',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar); 
      $this->Cell($anchura*3.5,$altura,'NRO PRODUCCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'FECHA DOCUMENTO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2.5,$altura,'ARTÍCULO',1,0,'L',$color_fondo);
      $this->Cell($anchura*4.5,$altura,'CANTIDAD A PRODUCIR',1,0,'L',$color_fondo);
  ...
  function NbLines($w,$txt)
  {
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(35,40,25,45,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();

    $sql="SELECT DISTINCT TRIM(nnro_produccion) nnro_produccion, TO_CHAR(dfecha_documento,'DD/MM/YYYY') dfecha_documento,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nnro_produccion']),
         ucwords($tperfil['dfecha_documento']),
         ucwords($tperfil['productoterminado']),
         ucwords($tperfil['cantidad_a_producir']),
