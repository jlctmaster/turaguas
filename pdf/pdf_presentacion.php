<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LAS PRESENTACIONES',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'PRESENTACIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'UNIDADES',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'CAPACIDAD',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,30,30,30,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT p.cdescripcion,p.nid_presentacion as id, p.nunidades as unidades, p.ncapacidad || ' ' || u.csimbolo as capacidad,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         ucwords($tperfil['unidades']),
         ucwords($tperfil['capacidad']),
