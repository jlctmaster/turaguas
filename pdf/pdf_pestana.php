<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS PESTAÑAS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar); 
      $this->Cell($anchura*4,$altura,'CÓDIGO PESTAÑA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'PESTAÑA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'CÓDIGO FORMULARIO',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'FORMULARIO',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,40,40,40,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT s.nid_servicio,LOWER(s.cnombreservicio) as servicio, f.nid_formulario,LOWER(f.cnombreformulario) as formulario, 
      CASE WHEN s.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus  
      FROM seguridad.tservicio s 
      INNER JOIN seguridad.tformulario f ON f.nid_formulario = s.nid_formulario 
