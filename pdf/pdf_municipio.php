<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS MUNICIPIOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*4,$altura,'CÓDIGO MUNICIPIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'MUNICIPIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'CÓDIGO CIUDAD',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'CIUDAD',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,30,40,30,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT m.nid_localidad_padre,LOWER(m.cdescripcion) municipio,m.nid_localidad,LOWER(c.cdescripcion) ciudad, 
    CASE WHEN m.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus  
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_localidad']),
         ucwords($tperfil['municipio']),
         ucwords($tperfil['nid_localidad_padre']),
         ucwords($tperfil['ciudad']),
