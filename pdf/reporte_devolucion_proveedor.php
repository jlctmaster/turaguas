   ...
   //Cabecera de página
    ...
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'CANTIDAD DE NOTAS DE DEVOLUCIÓN POR PROVEEDOR',0,1,"C");
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*3.5,$altura,'NRO. DEVOLUCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2.5,$altura,'FECHA DEV.',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'NRO. RECEPCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'FECHA RECEPCIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*5,$altura,'PROVEEDOR',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CANT. ARTÍCULOS',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4.5,$altura,'MOTIVO DEVOLUCIÓN',1,1,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(35,25,35,40,50,35,45));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT TRIM(dc.nnro_devolucion) nnro_devolucion, TO_CHAR(dc.dfecha_devolucion,'DD/MM/YYYY') dfecha_devolucion,
            TRIM(e.nnro_recepcion) nnro_recepcion, TO_CHAR(e.dfecha_documento,'DD/MM/YYYY') dfecha_recepcion, 
            p.crif_persona||' '||p.cnombre proveedor,md.cdescripcion motivodevolucion,
            COUNT(dc.cid_articulo) cantidad
            FROM facturacion.vw_devolucion_proveedor dc 
            INNER JOIN facturacion.vw_recepcion e ON dc.nid_documentocompra = e.nid_documentocompra AND dc.cid_articulo = e.cid_articulo 
            INNER JOIN general.tpersona p ON e.crif_persona = p.crif_persona 
            INNER JOIN facturacion.tmotivo_devolucion md ON dc.nid_motivodevolucion = md.nid_motivodevolucion WHERE e.dfecha_recepcion between '$dfecha_desde' and '$dfecha_hasta'
            GROUP BY dc.nnro_devolucion,dc.dfecha_devolucion,e.nnro_recepcion,e.dfecha_documento,p.crif_persona,p.cnombre,md.cdescripcion";   
$i=-1;
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         while($tperfil=$pgsql->Respuesta($data)){
          $cant++;
         $lobjPdf->Row(array(
         ucwords($tperfil['nnro_devolucion']),
         ucwords($tperfil['dfecha_devolucion']),
         ucwords($tperfil['nnro_recepcion']),
         ucwords($tperfil['dfecha_recepcion']),
         ucwords($tperfil['proveedor']),
	 ucwords($tperfil['cantidad']),
         ucwords($tperfil['motivodevolucion'])));
          
          $lobjPdf->Cell($avnzar);         
         }
        $lobjPdf->SetWidths(array(35,45));
        $lobjPdf->Cell($avnzar+175);
        $lobjPdf->SetFont("arial","B",10);
