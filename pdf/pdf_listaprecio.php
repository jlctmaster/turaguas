<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE PRECIOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*5,$altura,'VIGENCIA DESDE',1,0,'L',$color_fondo);
      $this->Cell($anchura*5,$altura,'VIGENCIA HASTA',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,30,50,50,26));
  require_once("../clases/class_bd.php");
    $pgsql=new Conexion();
    $sql="SELECT cdescripcion as cdescripcionlistaprecio,nid_listaprecio,TO_CHAR(dvigencia_desde,'DD/MM/YYYY') dvigencia_desde,TO_CHAR(dvigencia_hasta,'DD/MM/YYYY') dvigencia_hasta,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         (ucwords($tperfil['nid_listaprecio'])),
         (ucwords($tperfil['cdescripcionlistaprecio'])),
         (ucwords($tperfil['dvigencia_desde'])),
         (ucwords($tperfil['dvigencia_hasta'])),
