   ...
   //Cabecera de página
    ...
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'CANTIDAD DE NOTAS DE ENTREGAS POR CLIENTE',0,1,"C");
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*4,$altura,'RIF. CLIENTE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*5,$altura,'NOMBRE CLIENTE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*5,$altura,'DIRECCIÓN CLIENTE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'FECHA COTIZACIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'FECHA ENTREGA',1,1,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(40,40,50,50,40,35));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT d.nnro_entrega, cliente.crif_persona, cliente.cnombre, cliente.cdireccion, 
    to_char(d.dfecha_documento,'DD/MM/YYYY') fechacotizacion,to_char(d.dfecha_entrega,'DD/MM/YYYY') fechaentrega
From general.tpersona cliente 
Inner Join facturacion.vw_entrega d On cliente.crif_persona = d.crif_persona 
WHERE d.dfecha_entrega between '$dfecha_desde' and '$dfecha_hasta'";   
$i=-1;
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         ucwords($tperfil['fechacotizacion']),
         ucwords($tperfil['fechaentrega'])));
          
          $lobjPdf->Cell($avnzar);         
         }
        $lobjPdf->SetWidths(array(40,35));
