...
$this->Ln(55);  
   ...
   $this->Cell(0,6,'DETALLE DE LAS LISTAS DE PRECIOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3.5,$altura,'LISTA DE PRECIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'ARTÍCULO',1,0,'L',$color_fondo);
      $this->Cell($anchura*2,$altura,'PRECIO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'PRECIO LÍMITE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'DESCUENTO',1,1,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,35,30,20,30,30));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT d.nid_detallelistaprecio as nid_detallelistaprecio,d.nid_listaprecio as nid_listaprecio,l.cdescripcion as lista_precio,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_detallelistaprecio']),
         ucwords($tperfil['lista_precio']),
         ucwords($tperfil['cid_articulo']),
         ucwords($tperfil['nprecio']),
         ucwords($tperfil['nprecio_limite']),
         ucwords($tperfil['ndescuento'])));
          $lobjPdf->Cell($avnzar);         
         }
