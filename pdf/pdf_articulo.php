<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS ARTÍCULOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*4,$altura,'ARTÍCULOS',1,0,'L',$color_fondo); 
      $this->Cell($anchura*5,$altura,'TIPO DE ARTÍCULOS',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'CATEGORÍA',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'PRESENTACIÓN',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'IMPUESTO',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,40,50,40,40,40,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();   
    $sql="SELECT a.cid_articulo,a.cdescripcion as nom_articulo,ti.cdescripcion as nom_tipoarticulo,c.cdescripcion as nom_categoria,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         ucwords($tperfil['nom_articulo']),
         ucwords($tperfil['nom_tipoarticulo']),
         ucwords($tperfil['nom_categoria']),
         ucwords($tperfil['nom_presentacion']),
         ucwords($tperfil['nom_impuesto']),
