<?php
   ...
   if(isset($_POST['categoria_desde']))
    $categoria_desde=$_POST['categoria_desde'];

   if(isset($_POST['categoria_hasta']))
    $categoria_hasta=$_POST['categoria_hasta'];
   ...
   //Cabecera de página
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'INVENTARIO FÍSICO',0,1,"C");
...
function NbLines($w,$txt)
{
  ...
  $pgsql=new Conexion();
    $sql="SELECT dmi.cid_articulo||' '||a.cdescripcion AS articulo,cv.cdescripcion AS tipo,
    MAX(a.ncantidad_min) AS cnt_min,MAX(a.ncantidad_max) AS cnt_max,
    LAST(dmi.nvalor_actual) AS existencia
    ...
    INNER JOIN inventario.vw_categoria_materiales c ON a.nid_categoria = c.cod_subcategoria 
    INNER JOIN general.tcombo_valor cv ON dmi.ntipo_inventario = cv.nid_combovalor 
    WHERE c.cod_categoria between $categoria_desde AND $categoria_hasta 
    GROUP BY 1,2
    ORDER BY 1,2 ASC";
    $i=-1;
    $filas=array();
    ...
    if($pgsql->Total_Filas($data)!=0){
         while($tperfil=$pgsql->Respuesta($data)){
          $filas[$tperfil['tipo']]['articulo']= $tperfil['articulo'];
          $filas[$tperfil['tipo']]['cnt_min']= $tperfil['cnt_min'];
          $filas[$tperfil['tipo']]['cnt_max']= $tperfil['cnt_max'];
          $filas[$tperfil['tipo']]['existencia']= $tperfil['existencia'];
         }
         ...
         $sql2="SELECT DISTINCT cv.cdescripcion AS tipo 
          FROM inventario.tmovimiento_inventario mi 
          INNER JOIN inventario.tdetalle_movimiento_inventario dmi ON mi.nid_movinventario = dmi.nid_movinventario 
          INNER JOIN inventario.tarticulo a ON dmi.cid_articulo = a.cid_articulo 
          INNER JOIN inventario.vw_categoria_materiales c ON a.nid_categoria = c.cod_subcategoria 
          ...
          GROUP BY 1
          ORDER BY 1 ASC";
          $query=$pgsql->Ejecutar($sql2);
          $columns=array("ARTÍCULOS");
          $with=array("25");
