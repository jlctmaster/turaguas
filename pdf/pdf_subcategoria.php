<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LAS SUB-CATEGORÍAS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
      $this->Cell($anchura*4.5,$altura,'CÓD. SUB-CATEGORÍA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'SUB-CATEGORÍA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CÓD. CATEGORÍA',1,0,'L',$color_fondo);
      $this->Cell($anchura*2.5,$altura,'CATEGORÍA',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(45,35,35,25,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT s.nid_categoria_padre,LOWER(s.cdescripcion) sub,c.nid_categoria,LOWER(c.cdescripcion) categoria, 
    CASE WHEN s.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus  
    FROM inventario.tcategoria s 
    INNER JOIN inventario.tcategoria c ON c.nid_categoria = s.nid_categoria_padre 
    WHERE c.dfecha_desactivacion IS NULL 
    ORDER BY s.nid_categoria DESC";
   $i=-1;
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_categoria_padre']),
         ucwords($tperfil['sub']),
         ucwords($tperfil['nid_categoria']),
         ucwords($tperfil['categoria']),
