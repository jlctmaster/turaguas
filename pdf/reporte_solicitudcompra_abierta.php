   ...
   //Cabecera de página
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'SOLICITUDES DE COMPRAS ABIERTAS',0,1,"C");
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'N° SOLICITUD',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2,$altura,'FECHA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'ARTÍCULOS',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4.5,$altura,'CANTIDAD SOLICITADA',1,1,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(30,20,30,45));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();,
  
    $sql="SELECT DISTINCT TRIM(sc.nnro_solicitud) nnro_solicitud,TO_CHAR(r.dfecha_documento,'DD/MM/YYYY') fecha,
p.crif_persona||' '||p.cnombre proveedor,COUNT(r.cid_articulo) cantidad_solicitada, count(r.ncantidad_articulo) cantidad_recibida
...
Inner Join general.tpersona p On p.crif_persona = r.crif_persona WHERE sc.dfecha_documento BETWEEN '$dfecha_desde' AND '$dfecha_hasta' 
GROUP BY sc.nnro_solicitud,sc.cid_articulo,fecha,proveedor
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         while($tperfil=$pgsql->Respuesta($data)){
          $cant++;
         $lobjPdf->Row(array(
         ucwords($tperfil['nnro_solicitud']),
         ucwords($tperfil['fecha']),
         ucwords($tperfil['proveedor']),
         ucwords($tperfil['cantidad_solicitada']),
         ucwords($tperfil['cantidad_recibida'])));
          
          $lobjPdf->Cell($avnzar);         
         }
        $lobjPdf->SetWidths(array(30,45));
