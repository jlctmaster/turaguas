<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LA CONFIGURACIÓN DE ARTÍCULOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*2.5,$altura,'CÓD. INSUMO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'INSUMO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3.5,$altura,'CANTIDAD A USAR',1,0,'L',$color_fondo);
      $this->Cell($anchura*2,$altura,'MERMA',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,35,30,25,30,35,20,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT ca.nid_configuracionarticulo,TRIM(ca.cid_servicio) cid_servicio,ars.cdescripcion servicio, 
      TRIM(ca.cid_insumo) cid_insumo,ari.cdescripcion insumo,ca.ncantidad,ca.nmerma,
      CASE WHEN ca.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus 
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_configuracionarticulo']),
         ucwords($tperfil['cid_servicio']),
         ucwords($tperfil['servicio']),
         ucwords($tperfil['cid_insumo']),
         ucwords($tperfil['insumo']),
         ucwords($tperfil['ncantidad']),
         ucwords($tperfil['nmerma']),
