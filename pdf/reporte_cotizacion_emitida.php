   ...
   //Cabecera de página
    public function Header()
    {
      ...
      $this->Ln(55);  
       ...
       $this->Cell(0,6,'COTIZACIONES EMITIDAS POR CLIENTE',0,1,"C");
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'N° COTIZACIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2,$altura,'FECHA',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'RIF. CLIENTE',1,0,'L',$color_fondo); 
      $this->Cell($anchura*4,$altura,'NOMBRE CLIENTE',1,0,'L',$color_fondo);
      $this->Cell($anchura*2.5,$altura,'ARTÍCULOS',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'CANTIDAD COTIZADA',1,1,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(30,20,30,40,25,40));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT c.nnro_cotizacion,TO_CHAR(c.dfecha_documento,'DD/MM/YYYY') fecha,
    c.crif_persona,p.cnombre,COUNT(c.cid_articulo) articulo,SUM(ncantidad_articulo) cant 
    FROM facturacion.vw_cotizacion c 
    INNER JOIN general.tpersona p ON c.crif_persona = p.crif_persona 
    WHERE dfecha_documento between '$dfecha_desde' and '$dfecha_hasta' 
    GROUP BY c.nnro_cotizacion,c.dfecha_documento,c.crif_persona,p.cnombre 
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         while($tperfil=$pgsql->Respuesta($data)){
          $cant++;
         $lobjPdf->Row(array(
         ucwords($tperfil['nnro_cotizacion']),
         ucwords($tperfil['fecha']),
         ucwords($tperfil['crif_persona']),
         ucwords($tperfil['cnombre']),
         ucwords($tperfil['articulo']),
         ucwords($tperfil['cant'])));
          
          $lobjPdf->Cell($avnzar);         
         }
        $lobjPdf->SetWidths(array(25,40));
        $lobjPdf->Cell($avnzar*2.665);
        $lobjPdf->SetFont("arial","B",10);
