<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell (0,6,'LISTADO DE LOS IMPUESTOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'IMPUESTO',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3,$altura,'PORCENTAJE',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
  ...
  $pgsql=new Conexion();
    $sql="SELECT LOWER(cdescripcion) as impuesto,nid_impuesto as id, nporcentaje || '%' as porcentaje,
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['id']),
         ucwords($tperfil['impuesto']),
         ucwords($tperfil['porcentaje']),
