<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LAS NOTAS DE DEVOLUCIÓN PROVEEDOR',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*3,$altura,'FECHA DEV.',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'NRO. ENTREGA',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'FECHA DE ENT.',1,0,'L',$color_fondo);
      $this->Cell($anchura*5,$altura,'PROVEEDOR',1,0,'C',$color_fondo);
      $this->Cell($anchura*3.5,$altura,'CANT. ARTÍCULOS',1,0,'L',$color_fondo);
      $this->Cell($anchura*4,$altura,'MOTIVO DEVOLUCIÓN',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(35,30,30,30,50,35,40,20)); 
  ...
  $pgsql=new Conexion();
    $sql="SELECT TRIM(dp.nnro_devolucion) nnro_devolucion, TO_CHAR(dp.dfecha_devolucion,'DD/MM/YYYY') dfecha_devolucion,
            ...
            CASE WHEN dp.dfecha_devolucion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus
            ...
            INNER JOIN general.tcombo_valor v on p.ntipo_persona=v.nid_combovalor 
    WHERE lower(v.cdescripcion) LIKE '%proveedor%' 
