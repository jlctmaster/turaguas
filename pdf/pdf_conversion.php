<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LAS CONVERSIONES',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->Cell($avnzar); 
      $this->Cell($anchura*3.5,$altura,'CÓD. CONVERSIÓN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*3.5,$altura,'CÓD. ARTÍCULOS',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'ARTÍCULOS',1,0,'L',$color_fondo);
      $this->Cell($anchura*2.5,$altura,'U.M BASE',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'U.M DESTINO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3.5,$altura,'FACTOR MULTIPLO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3.5,$altura,'FACTOR DIVISOR',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(35,35,30,25,30,35,35,26));
  require_once("../clases/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT uc.nid_um_conversion,TRIM(uc.cid_articulo) cid_articulo,a.cdescripcion articulo, 
      ...
      CASE WHEN uc.dfecha_desactivacion IS NULL THEN 'Activo' ELSE 'Desactivado' END estatus
  ...
  $data=$pgsql->Ejecutar($sql);
    if($pgsql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         ucwords($tperfil['nid_um_conversion']),
         ucwords($tperfil['cid_articulo']),
         ucwords($tperfil['articulo']),
         ucwords($tperfil['um_base']),
         ucwords($tperfil['um_destino']),
         ucwords($tperfil['nfactor_multiplicador']),
         ucwords($tperfil['nfactor_divisor']),
