<div class="form_externo" >
  ...
  <fieldset style="padding: 30px">
    <form action="../controladores/control_lineaordencompra.php" method="post" id="form2">
      <table border="0" style="width: 100%">
        <tr>
          ...
          <td>
            <?php if(!empty($nid_documento)){?>
              ...
              <input title="El código del grupo prociene de la pestaña anterior" name="name_orden" id="name_orden" type="text" readonly value="<?= $nnro_orden."_".$dfecha_documento;?>" /> 
            <?php }else{?>
              <input type="hidden" id="nid_documento" name="nid_documento" value="<?= $_SESSION['nid_documento']; ?>">
              <input title="El nombre de la ubicación prociene de la pestaña anterior" name="name_orden" id="name_orden" type="text" readonly value="<?= $_SESSION['nnro_orden']."_".$_SESSION['dfecha_documento'];?>" /> 
            <?php } ?>
          </td>
        </tr>
        <tr>
          ...
          <td><input title="Correlativo para las líneas de la orden de compra" name="nlinea" id="nlinea" type="text" size="10" value="<? if(empty($nlinea)){echo "1";}else{echo $nlinea;}?>" /></td>
        </tr>
        <tr>
          <?php
        ...
        <tr>
          <td colspan="2" style="padding-top:12px;padding-bottom:12px; ">
            ...
            <?php
              imprimir_boton($disabledRCOCL,$disabledMDOCL,$estatusocl,$servicios,"lineaordencompra");
            ?>         
          </td>
        </tr>
      </table>
  ...
  <script type="text/javascript">
    function reload_tablinea(tab){
      ...
      location.href="menu_principal.php?ordencompra&l&cid_articulo="+cid_articulo+tab;
    }
  </script>
  <table border="0" style="width: 100%"> 
    <tr>
  ...
  <table align="center">  
    <tr>
      <td>
        <input type="button" class="btn btn-default" value="Buscar" onclick="reload_tablinea('#lineaordencompra')">
        <input type="button" class="btn btn-default" value="Limpiar" onclick="limpiar()">
      </td>
    </tr>
  </table> 
  <a href="?ordencompra#lineaordencompra" ><img src="../images/cerrar.png" alt="Cerrar" style="width:40px;heigth:40px;float:right;"></a>
  <a href="../excel/excel_lineaordencompra.php" ><img src="../images/icon-excel.png" alt="Exportar a Excel" style="width:40px;heigth:40px;float:right;"></a>
  <a href="<?php echo  '../pdf/?serv=lineaordencompra';?>" target="_blank"><img src="../images/icon-pdf.png" alt="Exportar a PDF" style="width:40px;heigth:40px;float:right;"></a>
  <table style="width:100%;" class="tablapaginacion">
    ...
    require_once("../clases/class_bd.php");
    $pgsql=new Conexion();
    $clausuleWhere = "WHERE d.nnro_orden = '".$_SESSION['nnro_orden']."' ";
    if($cid_articulo!=""){
    ...
    while($row = pg_fetch_array($_pagi_result)){ 
      echo "<tr style='cursor: pointer;' id='".$row['nid_documento']."|".$row['nlinea']."' onclick='enviarFormLOC(this.id)'>
  ...
  <form id="form2" method="POST" action="../controladores/control_lineaordencompra.php">
