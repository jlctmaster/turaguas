...
if(!isset($_GET['l'])) { ?>
     <fieldset style="padding: 30px">
          <form action="../controladores/control_contactocliente.php" method="post" id="form3">
        <table border="0" style="width: 100%">
         <tr> <td><span class="texto_form">Cliente</span></td>
          <td>
            ...
            <tr><td><span class="texto_form">Dirección de Despacho</span></td>
          <td>
             <select name="nid_direcciondespacho" id="nid_direcciondespacho" title="Seleccione una dirección de despacho" required>
              <option value="0">Seleccione una dirección de despacho</option>
              <?php
              require_once("../clases/class_bd.php");
              $conexion = new Conexion();
              if(!empty($crif_personacliente)){
                $sql="SELECT nid_direcciondespacho, cdireccion FROM general.tdireccion_despacho WHERE crif_persona = '$crif_personacliente' ORDER BY nid_direcciondespacho ASC";
              }else{
                $sql="SELECT nid_direcciondespacho, cdireccion FROM general.tdireccion_despacho WHERE crif_persona = '".$_SESSION['crif_personacliente']."' ORDER BY nid_direcciondespacho ASC";
              }
...
<br>
  ...
  function reload_tabcontacto(tab){
    ...
    cnombrecontacto = document.getElementById('cnombrecontacto').value;
        location.href="menu_principal.php?cliente&l&crif_personacontacto="+crif_personacontacto+"&cnombrecontacto="+cnombrecontacto+tab;
  }
</script>
    ...
    <tr>
      ...
      <td>
        <input onKeyPress="return isRif(event)" onKeyUp="this.value=this.value.toUpperCase()" title="Ingrese el RIF del contacto" name="crif_personacontacto" id="crif_personacontacto" type="text" size="10" value="<?= $crif_personacontacto;?>" />
      </td>
  ...
  <a href="?cliente#contacto"><img src="../images/cerrar.png" alt="Cerrar" style="width:40px;heigth:40px;float:right;"></a>
  ...
  <a href="<?php echo  '../pdf/?serv=contactocliente';?>" target="_blank"><img src="../images/icon-pdf.png" alt="Exportar a PDF" style="width:40px;heigth:40px;float:right;"></a>
     <table style="width:100%;" class="tablapaginacion">
           <tr style="background: #000; color:#FFF; width:100%;"> 
               <td style="width:20%;"> RIF</td>
...
$clausuleWhere = "WHERE p.crif_persona = '".$_SESSION['crif_personacliente']."'";
...
//Incluimos la barra de navegación 
  ...
  <form id="form3" method="POST" action="../controladores/control_contactocliente.php">
