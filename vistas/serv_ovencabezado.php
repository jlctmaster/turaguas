<div class="form_externo" >
  ...
  if(isset($_SESSION['datos']['nnro_orden'])){ 
    $disabledRCOV='disabled';
    $disabledMDOV='';
    $estatusov=null;
  }else {
    $disabledRCOV='';
    $disabledMDOV='disabled';
  }
  ...
  if(isset($_SESSION['datos'])){
    ...
    @$estatusov=$_SESSION['datos']['estatus'];
  }
  else{
    @$nid_documento=null;
    @$nnro_orden=null;
  ...
  <fieldset style="padding: 30px">
    <form action="../controladores/control_ordenventa.php" method="post" id="form1">
      <table border="0" style="width: 100%">
          ...
          <td>
            <select name="nid_tipodocumento" id="nid_tipodocumento" title="Seleccione un tipo de documento" required />
              ...
              <?php
                ...
                AND dfecha_desactivacion IS NULL AND (LOWER(cdescripcion) LIKE '%ov%' OR LOWER(cdescripcion) LIKE '%órden de venta%' OR LOWER(cdescripcion) LIKE '%venta%')
        ...
        <tr>
          ...
          <td>
            <input title="Ingrese el nro. de la órden de venta" onKeyPress="return isNumberDocKey(event)" name="nnro_orden" 
        ...
        <tr>
          ...
          <td>
            <input title="Seleccione la fecha de la órden de venta" name="dfecha_documento" id="dfecha_documento" type="text" size="50" readonly value="<?= $dfecha_documento;?>" required />
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td>
            <input title="Ingrese el rif o cédula del cliente" onKeyUp="this.value=this.value.toUpperCase()" name="crif_persona" id="crif_persona" type="text" size="50" value="<?= $crif_persona;?>" required />
          </td>
        </tr>
        <tr>
          <td>
        ...
        <tr>
          ...
          <td>
            <textarea title="Dirección de facturación del cliente" name="cdireccioncliente" id="cdireccioncliente" rows="5" cols="50"  readonly /><?= $cdireccioncliente;?> </textarea>
          </td>
        </tr>
        <tr>
          <td>
        ...
        <tr>
          ...
          <td>
            <input title="El monto total a pagar es calculado según las líneas del documento más el impuesto" name="nmonto_total" id="nmonto_total" type="text" size="50" readonly value="<? if(empty($nmonto_total)){echo "0";}else{echo $nmonto_base;}?>" required />
          </td>
        </tr>
        <tr>
          <td>
        ...
        <?php echo '<tr><td colspan="2" class="'.$estatusov.'" id="estatus_registro">'.$estatusov.'</td></tr>'; ?>
        <tr>
          <td colspan="2" style="padding-top:12px;padding-bottom:12px; ">
                    <span class="texto_form"> Los campos resaltados en rojo son obligatorios </span><br /><br />
            <?php
              imprimir_boton($disabledRCOV,$disabledMDOV,$estatusov,$servicios,"ordenventa");
            ?>
            <input onclick="location='../invoice/ordencv.php?nnro_orden=<?= $nnro_orden;?>&tipo=V'" 
  ...
  <script type="text/javascript">
    function reload_tabov(tab){
      ...
      nnro_orden = document.getElementById('nnro_orden').value;
      location.href="menu_principal.php?ordenventa&l&nnro_orden="+nnro_orden+tab;
    }
  </script>
  <table border="0" style="width: 100%"> 
    <tr>
      ...
      <td>
        <input title="Ingrese el nro. de la órden de venta" onKeyUp="this.value=this.value.toUpperCase()" name="nnro_orden" id="nnro_orden" type="text" size="50" value="<?= $nnro_orden;?>" />
      </td>
      <td>
  ...
  <table align="center">  
    <tr>
      <td>
        <input type="button" class="btn btn-default" value="Buscar" onclick="reload_tabov('#ordenventa')">
  ...
  <a href="<?php echo  '../pdf/?serv=ordenventa';?>" target="_blank"><img src="../images/icon-pdf.png" alt="Exportar a PDF" style="width:40px;heigth:40px;float:right;"></a>
  <table style="width:100%;" class="tablapaginacion">
    ...
    require_once("../clases/class_bd.php");
    $pgsql=new Conexion();
    $clausuleWhere="WHERE d.dfecha_desactivacion IS NULL AND d.ctipo_transaccion = 'V' ";
    ...
    $_pagi_sql = "SELECT d.nnro_orden, td.cdescripcion tipodocumento, TO_CHAR(d.dfecha_documento,'DD/MM/YYYY') dfecha_documento, 
    ...
    while($row = pg_fetch_array($_pagi_result)){ 
      echo "<tr  style='cursor: pointer;' id='".$row['nnro_orden']."' onclick='enviarFormOV(this.id)'>
  ...
  <script type="text/javascript">
  function enviarFormOV(value){
    document.getElementById('nnro_orden_oculto').value=value;
    document.getElementById('form1').submit();
  }
  </script>
  <form id="form1" method="POST" action="../controladores/control_ordenventa.php">
